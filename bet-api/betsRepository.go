...
func GetBetById(betId string) *Bet {
	bet := &Bet{}
	err := repository.GetDB().Table("bets").Where("bet_id =?", betId).First(bet).Error
...
func GetAllBets() []*Bet {
	bet := make([]*Bet, 0)
	err := repository.GetDB().Find(&bet).Error
	if err != nil {
		return nil
	}
	return bet
...
func GetBetsByUserId(id string) []*Bet {
	bets := make([]*Bet, 0)
	err := repository.GetDB().Table("bets").Where("user_id = ?", id).Find(&bets).Error
	if err != nil {
		fmt.Println(err)
		return nil
	}
...
func GetBetsByChallengeId(id string) []*Bet {
	bets := make([]*Bet, 0)
	err := repository.GetDB().Table("bets").Where("challenge_id = ?", id).Find(&bets).Error
	if err != nil {
		fmt.Println(err)
		return nil
	}
