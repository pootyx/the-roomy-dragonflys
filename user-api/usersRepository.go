...
func (user *User) Authorize() (map[string]interface{}, bool) {
	...
	err := repository.GetDB().Table("users").Where("email = ?", user.Email).First(temp).Error
...
func Login(email, password string) map[string]interface{} {
	...
	user := &User{}
	err := repository.GetDB().Table("users").Where("email = ?", email).First(user).Error
	if err != nil {
...
func GetAllUsers() []*User {
	users := make([]*User, 0)
	err := repository.GetDB().Find(&users).Error
	if err != nil {
		return nil
	}
	return users
...
func GetUserById(userId string) *User {
	user := &User{}
	err := repository.GetDB().Table("users").Where("user_id = ?", userId).First(user).Error
...
func DeleteUserById(userId string) {
	repository.GetDB().Table("users").Where("user_id = ?", userId).Delete(&User{})
}
